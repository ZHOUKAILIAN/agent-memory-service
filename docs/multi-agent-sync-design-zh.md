...
- 共享服务端数据库使用 `PostgreSQL`
- 本地 bridge 数据库使用 `SQLite`

分工如下：

- `PostgreSQL` 负责项目、任务、checkpoint、detail memory、artifact refs 等共享数据
- `SQLite` 负责 workspace 绑定、outbox、本地 cache、本地 session events、全文检索
...
如果本地 bridge 使用 `SQLite`，也可以把这个绑定信息存进本地库，而不是只放 json 文件。
...
如果 bridge 使用 `SQLite`，也可以把 outbox 事件和重试状态持久化到本地库里。
